...
export function Dashboard({ onNavigate }: { onNavigate: (p: string) => void }) {
  ...
  const stats = [
    { label: "Active Downloads", value: activeDownloads.value.length, color: "text-blue-400", bg: "bg-blue-500/8 border-blue-500/15", icon: (
    ...
    { label: "Active Transcodes", value: activeTranscodes.value.length, color: "text-purple-400", bg: "bg-purple-500/8 border-purple-500/15", icon: (
      <svg class="w-5 h-5 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
    )},
    { label: "Completed Downloads", value: completedDownloads.value.length, color: "text-emerald-400", bg: "bg-emerald-500/8 border-emerald-500/15", icon: (
    ...
    { label: "Completed Transcodes", value: completedTranscodes.value.length, color: "text-teal-400", bg: "bg-teal-500/8 border-teal-500/15", icon: (
  ...
  return (
    <div class="space-y-6 animate-fade-in">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div key={s.label} class={`rounded-xl border p-4 ${s.bg} flex items-start gap-3`}>
            <div class="mt-0.5">{s.icon}</div>
            <div>
              <p class="text-xs text-surface-400 font-medium">{s.label}</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-surface-900/50 border border-surface-800/50 rounded-xl overflow-hidden">
          <div class="flex items-center justify-between px-5 py-3.5 border-b border-surface-800/50">
            ...
            <button onClick={() => onNavigate("downloads")} class="text-xs text-blue-400 hover:text-blue-300 font-medium transition-colors">View all</button>
          </div>
          <div class="divide-y divide-surface-800/50">
            {recentDownloads.length === 0 && (
              <div class="px-5 py-10 text-center">
                ...
                <p class="text-sm text-surface-500">No downloads yet</p>
              </div>
            )}
        ...
        <div class="bg-surface-900/50 border border-surface-800/50 rounded-xl overflow-hidden">
          <div class="flex items-center justify-between px-5 py-3.5 border-b border-surface-800/50">
            ...
            <button onClick={() => onNavigate("queue")} class="text-xs text-purple-400 hover:text-purple-300 font-medium transition-colors">View all</button>
          </div>
          <div class="divide-y divide-surface-800/50">
            {recentQueue.length === 0 && (
              <div class="px-5 py-10 text-center">
                ...
                <p class="text-sm text-surface-500">No queue tasks yet</p>
              </div>
            )}
            {recentQueue.map((t) => (
